...
char* char2mace(char x){
  switch(x){
    ...
    //Others
     ...
     case ' ':
       return " ";
     default : // Incase a character can not be translated into mace
       return "~";
  }
}
  ...
  if (input[0] == '$' || input[0] == '^' || input[0] == '='){ // If the code is mace code
    ...
    if (value == "RXIR"){
      ...
      Serial.println("IR SENT");
       ...
       //Read the analog value from the potentiometer and transmit it on the serial
        //port as 4 MACE encoded decimal digits.
    }
    else if (value.startsWith("LP")){
      int intensity = 000;
      if (value.length() == 14){
        for (int digit = 2; digit < 14; digit++){
...
void macestring2LEDred(String input){
  int loopcounter = 0;
  while (true){
    if (input[loopcounter] == '/'){
      if (input[loopcounter + 1] == '/'){
    ...
    sendLedSignalRed(input[loopcounter]);
    loopcounter ++;
  }
}
...
void sendLedSignalRed(char sim){
  if (sim=='^'){
    red_on();
    delay(1*potReadingToDelay());
    red_off();
    ...
    red_off(); // fixes a bug which im not sure why its being caused
    delay(2*potReadingToDelay());
  }
  else if (sim==' '){
    red_off(); // same here
